...
shape_acceleration_raw = [shape_velocity_raw(:, 2) - shape_velocity_raw(:, 1), ...
    shape_velocity_raw(:, 3:end) - shape_velocity_raw(:, 1:end-2), ...
    shape_velocity_raw(:, end) - shape_velocity_raw(:, end-1)] ./ ...
    [time_resample(2)-time_resample(1), time_resample(3:end) - time_resample(1:end-2), time_resample(end)-time_resample(end-1)];
...
position_acceleration_raw = [position_velocity_raw(:, 2) - position_velocity_raw(:, 1), ...
    position_velocity_raw(:, 3:end) - position_velocity_raw(:, 1:end-2), ...
    position_velocity_raw(:, end) - position_velocity_raw(:, end-1)] ./ ...
    [time_resample(2)-time_resample(1), time_resample(3:end) - time_resample(1:end-2), time_resample(end)-time_resample(end-1)];

command_dot_raw = [command_raw(:, 2) - command_raw(:, 1), ...
    command_raw(:, 3:end) - command_raw(:, 1:end-2), ...
    command_raw(:, end) - command_raw(:, end-1)] ./ ...
    [time_resample(2)-time_resample(1), time_resample(3:end) - time_resample(1:end-2), time_resample(end)-time_resample(end-1)];
force_dot_raw = [force_raw(:, 2) - force_raw(:, 1), ...
    force_raw(:, 3:end) - force_raw(:, 1:end-2), ...
    force_raw(:, end) - force_raw(:, end-1)] ./ ...
    [time_resample(2)-time_resample(1), time_resample(3:end) - time_resample(1:end-2), time_resample(end)-time_resample(end-1)];
...
position_acceleration = position_acceleration(:, idx);
shape = shape(:, idx);
shape_velocity = shape_velocity(:, idx);
shape_acceleration = shape_acceleration(:, idx);
force = force(:, idx);
force_dot = force_dot(:, idx);
...
save([bag_name(1:end-4), '.mat'], ...
    ...
    'shape_velocity_raw', ...
    'shape_velocity', ...
    'shape_acceleration_raw', ...
    'shape_acceleration', ...
    'position_raw', ...
    'position', ...
    'position_velocity_raw', ...
    'position_velocity', ...
    'position_acceleration_raw', ...
