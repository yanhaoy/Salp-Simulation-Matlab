...
mass_wheel = 0.177;
mass_caster = 0.316;
sys.config.M_local = zeros(sys.config.n * (sys.config.m + 1), 1);
for i = 1:sys.config.m + 1
    ...
    sys.config.M_local((i-1)*sys.config.n+1:i*sys.config.n) = [mass_actuator + mass_wheel + mass_caster; ...
        mass_actuator + mass_wheel + mass_caster; ...
        norm(g_actuator(1:2, 3))^2 * mass_actuator + norm(g_wheel(1:2, 3))^2 * mass_wheel + norm(g_caster(1:2, 3))^2 * mass_caster];
end
sys.config.M_local = diag(sys.config.M_local);
